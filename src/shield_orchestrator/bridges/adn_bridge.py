...
class ADNBridge(BaseLayer):
    """
    ADN bridge (Active Defense Network adapter).

    - Preserves legacy BaseLayer.process() behavior for pipeline.py.
    - Adds a v3-facing evaluation method emitting deterministic TraceEntry
      for Orchestrator v3 aggregation.

    NOTE (Phase 3 in-progress):
    This is a deterministic stub. Real ADN v3 integration will replace
    the internals while keeping this contract stable.
    """
    ...
    def evaluate_v3(self, request: OrchestratorV3Request) -> TraceEntry:
        """
        Evaluate ADN for a v3 orchestrator request.
